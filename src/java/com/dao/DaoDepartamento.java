...
public class DaoDepartamento extends Conexion{
    ...
    public void insertar(Departamento dep){
        ...
        try {
            if(this.conectar()==true){
                sql = "insert into departamento(nombre, cantidadEmpleado) values(?,?)";
    ...
    public void eliminar(Departamento dep){
        ...
        try {
            if(this.conectar()){
                ...
                sql = "delete from departamento where codigoDepartamento=?";
                pr = this.getConn().prepareStatement(sql);
    ...
    public void modificar(Departamento dep){
        ...
        try {
            if(this.conectar()){
                sql = "update departamento set nombre=?, cantidadEmpleado=? "
    ...
    public Departamento leerId(Departamento dep){
        ...
        try {
            if(this.conectar()){
                pr = this.getConn().prepareStatement("select codigoDepartamento, nombre, cantidadEmpleado "
                        + "from departamento where codigoDepartamento=?");
                pr.setInt(1, dep.getCodigoDepartamento());
                rs = pr.executeQuery();
